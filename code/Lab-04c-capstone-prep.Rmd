...
logistic_model2 <- glm(pd ~ entropy2 + rate_speech_time2 + accel_speech_time2 + dur_pause_int2 + dur_voice_int2 + gap_voiced_int2 + dur_unvoiced_stops2 + decay_fric2 + rel_loud_resp2 + pause_int_resp2 + rate_speech_resp2 + latency_resp2 + entropy1 + rate_speech_time1 + accel_speech_time1 + dur_pause_int1 + dur_voice_int1 + gap_voiced_int1 + dur_unvoiced_stops1 + decay_fric1 + rel_loud_resp1 + pause_int_resp1 + rate_speech_resp1 + latency_resp1, family= binomial, data=train_data)
...
model <- glm(pd ~ rate_speech_time1 + dur_pause_int1 + dur_pause_int2, family= binomial, data=train_data)
...
true_labels <- test_data$pd

# Using pROC
roc_curve <- roc(true_labels, predicted_probs)
plot(roc_curve, main = "ROC Curve (pROC)", col = "deepskyblue", lwd = 2)
auc_value <- auc(roc_curve)
print(paste("AUC (pROC):", auc_value))

# Using ROCR
prediction_obj <- prediction(predicted_probs, true_labels)
performance_obj <- performance(prediction_obj, "tpr", "fpr")
plot(performance_obj, col = "deepskyblue", lwd = 2, main = "ROC Curve (ROCR)")
auc_value_rocr <- performance(prediction_obj, measure = "auc")
auc_value_rocr <- auc_value_rocr@y.values[[1]]
print(paste("AUC (ROCR):", auc_value_rocr))
```
...
park.train <- select(train_data, pd, 45:68)

parkRF <- select(parkinsons_dataset, pd, 45:68)
library(ranger)
train.idx <-sample(nrow(parkRF), 2/3 * nrow(parkRF))
park.train <- parkRF[train.idx, ]
park.test <- parkRF[-train.idx, ]
park.rg <- ranger(pd~ . , data = train_data, importance = "impurity")
park.pred <- predict(park.rg, data = test_data)
table(park.test$pd, park.pred$predictions)
vars <- park.rg$variable.importance
...
dur_pause_int2
     ...
     entropy1         rate_speech_time1  accel_speech_time1      dur_pause_int1 
          1.0954938           1.2262261           1.0097911           1.3290853 
     dur_voice_int1     gap_voiced_int1 dur_unvoiced_stops1         decay_fric1 
          1.0610927           1.2642894           0.9405020           1.0635630 
     rel_loud_resp1     pause_int_resp1   rate_speech_resp1       latency_resp1 
          1.2638849           0.7517284           1.2588394           0.8801236 
           entropy2   rate_speech_time2  -------------      -------------- 
          0.7465149           1.5478808           3.0226179           2.3228441 
     dur_voice_int2     gap_voiced_int2 dur_unvoiced_stops2         decay_fric2 
          1.2935573           0.9230569           1.1129492           1.7472662 
     rel_loud_resp2     pause_int_resp2   rate_speech_resp2       latency_resp2 
          0.7874687           0.6981478           1.2894073           1.8770645 
```
