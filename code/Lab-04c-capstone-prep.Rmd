...
ggplot(data = parkinsons_dataset,
       aes(x = age, y = speech )) +
  geom_boxplot() +
  theme_bw ()

parkinsons_dataset %>% 
  drop_na(tremorRest_head) %>% 
       aes(x = fam_history) +
  geom_bar() +
  theme_bw ()
...
ggplot(data = parkinsons_dataset,
       aes(x = age_onset)) +
  geom_histogram(binwidth = 1, fill = "slategrey") +
  theme_bw ()

ggplot(data = parkinsons_dataset,
       aes(x = updrs, y = diagnosis)) +
...
ggplot(data = parkinsons_dataset,
       aes(x = age, color = post_stability)) +
  geom_histogram(fill = "white") +
  theme_bw ()

ggplot(parkinsons_dataset,
       aes(x = age_onset,
           y = duration,
           color = benz_meds)) +
  geom_point() +
...
ggplot(parkinsons_dataset, 
       aes(diagnosis, age, colour = facial)) + 
  geom_jitter() + 
theme_bw()

parkinsons_dataset %>% 
  drop_na(tremorRest_head) %>% 
  ggplot(aes(x = diagnosis, y = age, color = tremorRest_head)) + 
geom_jitter(stat = "identity", fill = "maroon4")

parkinsons_dataset %>% 
  drop_na(tremorRest_head) %>% 
  ggplot(aes(x = diagnosis, y = age_onset, color = entropy2)) + 
geom_jitter(stat = "identity", weight = 4) +
  labs(x = "Diagnosis",
  y = "Age Onset",
  title = "Entropy 2 of each diagnosis group") +
theme_bw()

parkinsons_dataset %>% 
  drop_na(tremorRest_head) %>% 
  ggplot(aes(x = diagnosis, y = age_onset, color = accel_speech_time1)) + 
geom_jitter(stat = "identity") +
  labs(x = "Diagnosis",
  y = "Age Onset",
  title = "Accelerated Speech Time of each diagnosis group") +
theme_bw()

parkinsons_dataset %>% 
  drop_na(posture) %>% 
  ggplot(aes(x = age_onset, y = posture, color = diagnosis)) + 
geom_jitter(stat = "identity") +
theme_bw()

parkinsons_dataset %>% 
  drop_na(posture) %>% 
  ggplot(aes(x = duration, y = age_onset, color = diagnosis)) + 
geom_jitter(stat = "identity") +
...
```{r}
#more graphs
attach(parkinsons_dataset)
plot(diagnosis,rigid_neck, main="Scatterplot of diagnosis neck rigidity")
plot(diagnosis,rigid_RUE, main="Scatterplot of diagnosis RUE rigidity")
plot(diagnosis,rigid_LUE, main="Scatterplot of diagnosis LUE rigidity")
plot(diagnosis,rigid_RLE, main="Scatterplot of diagnosis RLE rigidity")
plot(diagnosis,rigid_LLE, main="Scatterplot of diagnosis LLE rigidity")

plot(diagnosis,fingertaps_RUE, main="Scatterplot of diagnosis fingertaps_RUE")
plot(diagnosis,fingertaps_LUE, main="Scatterplot of diagnosis fingertaps_LUE")
plot(diagnosis,handmove_RUE, main="Scatterplot of diagnosis handmove_RUE")
plot(diagnosis,handmove_LUE, main="Scatterplot of diagnosis handmove_LUE")

plot(diagnosis,rapidAltMove_RUE, main="Scatterplot of diagnosis rapidAltMove_RUE")
plot(diagnosis,rapidAltMove_LUE, main="Scatterplot of diagnosis rapidAltMove_LUE")
plot(diagnosis,legAg_RLE, main="Scatterplot of diagnosis legAg_RLE")
plot(diagnosis,legAG_LLE, main="Scatterplot of diagnosis legAG_LLE")

plot(pd,dur_pause_int1, main="Scatterplot of diagnosis and dur_pause_int1")
plot(pd,dur_voice_int1, main="Scatterplot of diagnosis and dur_voice_int1")
plot(pd,gap_voiced_int1, main="Scatterplot of diagnosis and gap_voiced_int1")
plot(pd,dur_pause_int2, main="Scatterplot of diagnosis and dur_pause_int2")
plot(pd,dur_voice_int2, main="Scatterplot of diagnosis and dur_voice_int2")
plot(pd,gap_voiced_int2, main="Scatterplot of diagnosis and gap_voiced_int2")
```

```{r}
#t-tests and p-values maybe?
summary(parkinsons_dataset)

lm(age_onset ~ diagnosis + entropy1 + entropy2 + posture + gait + post_stability + brady_hypo + chair, data = parkinsons_dataset) %>%
  tbl_regression(estimate_fun = function(x) style_number(x, digits = 3))

lm(age_onset ~ diagnosis + rate_speech_time1 + accel_speech_time1 + dur_pause_int1 + dur_voice_int1 + gap_voiced_int1 + dur_unvoiced_stops1 + decay_fric1 + rel_loud_resp1 + pause_int_resp1 + rate_speech_resp1 + latency_resp1, data = parkinsons_dataset) %>%
  tbl_regression(estimate_fun = function(x) style_number(x, digits = 3))

lm(age_onset ~ diagnosis + rate_speech_time2 + accel_speech_time2 + dur_pause_int2 + dur_voice_int2 + gap_voiced_int2 + dur_unvoiced_stops2 + decay_fric2 + rel_loud_resp2 + pause_int_resp2 + rate_speech_resp2 + latency_resp2, data = parkinsons_dataset) %>%
  tbl_regression(estimate_fun = function(x) style_number(x, digits = 3))


pd_model <- lm(age_onset ~ diagnosis + brady_hypo + entropy1 + entropy2, data = parkinsons_dataset)
summary(pd_model)

pd_model <- lm(age_onset ~ diagnosis + chair + posture + gait + post_stability, data = parkinsons_dataset)
summary(pd_model)

#***
pd_model <- lm(age_onset ~ age + duration, data = parkinsons_dataset)
summary(pd_model) 

pd_model <- lm(duration ~ tremorRest_head + tremorRest_RUE + tremorRest_LUE+tremorRest_RLE+tremorRest_LLE+AP_tremor_RUE+AP_tremor_LUE+rigid_neck+rigid_RUE+rigid_LUE+rigid_RLE+rigid_LLE+fingertaps_RUE+fingertaps_LUE+handmove_RUE+handmove_LUE+rapidAltMove_RUE+rapidAltMove_LUE+legAg_RLE+legAG_LLE + rate_speech_time1 + accel_speech_time1 + dur_pause_int1 + dur_voice_int1 + gap_voiced_int1 + dur_unvoiced_stops1 + decay_fric1 + rel_loud_resp1 + pause_int_resp1 + rate_speech_resp1 + latency_resp1 + rate_speech_time2 + accel_speech_time2 + dur_pause_int2 + dur_voice_int2 + gap_voiced_int2 + dur_unvoiced_stops2 + decay_fric2 + rel_loud_resp2 + pause_int_resp2 + rate_speech_resp2 + latency_resp2, data = parkinsons_dataset)
summary(pd_model) 


glm(formula = diagnosis ~ age, family = binomial,
    data = parkinsons_dataset)

glm(formula = diagnosis ~ facial + speech + updrs + gait, family = binomial,
    data = parkinsons_dataset)
```
...
logistic_model_voice1 <- glm(pd ~ age + gender + entropy1 + rate_speech_time1 + accel_speech_time1 + dur_pause_int1 + dur_voice_int1 + gap_voiced_int1 + dur_unvoiced_stops1 + decay_fric1 + rel_loud_resp1 + pause_int_resp1 + rate_speech_resp1 + latency_resp1, family = binomial, data = train_data)
summary(logistic_model_voice1)

logistic_model_voice2 <- glm(pd ~ age + gender + entropy2 + rate_speech_time2 + accel_speech_time2 + dur_pause_int2 + dur_voice_int2 + gap_voiced_int2 + dur_unvoiced_stops2 + decay_fric2 + rel_loud_resp2 + pause_int_resp2 + rate_speech_resp2 + latency_resp2, family = binomial, data = train_data)
...
#fix whatever this thing is
#pred_prob_mean <- mean(pred_prob1, pred_prob2)
