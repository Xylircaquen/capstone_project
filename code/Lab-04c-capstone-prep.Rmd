...
```{r}
# no association between age and duration of disease
parkinsons_dataset %>% 
  ggplot(aes(x = age, y = duration, color = diagnosis)) +
  geom_point()

# people with PD have more severe speech and facial problems
parkinsons_dataset %>% 
  drop_na(speech, facial) %>% 
  ggplot(aes(x = speech, y = facial, color = diagnosis)) +
  geom_jitter()

# there are not enough people with a family history of PD to prove a relationship
parkinsons_dataset %>% 
  drop_na(fam_history) %>% 
  ggplot(aes(x = fam_history, y = age_onset)) +
  geom_boxplot()

# matrix plot of tremor at rest
parkinsons_dataset[,c(2, 18:22)] %>% 
  drop_na() %>% 
  ggpairs(aes(color = diagnosis))

# matrix plot of posture, gait, postural stability, bradykinesia and hypokinesia
parkinsons_dataset[,c(2, 39:42)] %>% 
  drop_na() %>% 
  ggpairs(aes(color = diagnosis))

# there are more people with PD that don't have good postural stability
# there is one person with RBD that has worse post stability than everyone else (outlier?)
parkinsons_dataset %>% 
  drop_na(post_stability) %>% 
  ggplot(aes(x = post_stability, y = diagnosis, color = diagnosis)) +
  geom_jitter()
```
...
# voice variables eda
parkinsons_dataset %>% 
  ggplot(aes(x = id, y = dur_unvoiced_stops1)) +
  geom_boxplot(aes(color = diagnosis)) +
  geom_point(aes(shape = diagnosis))

parkinsons_dataset %>% 
  ggplot(aes(x = id, y = dur_unvoiced_stops2)) +
  geom_boxplot(aes(color = diagnosis)) +
  geom_point(aes(shape = diagnosis))

parkinsons_dataset %>% 
  ggplot(aes(x = id, y = decay_fric1)) +
  geom_boxplot(aes(color = diagnosis)) +
  geom_point(aes(shape = diagnosis))

parkinsons_dataset %>% 
  ggplot(aes(x = id, y = decay_fric2)) +
  geom_boxplot(aes(color = diagnosis)) +
  geom_point(aes(shape = diagnosis))

parkinsons_dataset %>% 
  ggplot(aes(x = id, y = rel_loud_resp1)) +
  geom_boxplot(aes(color = diagnosis)) +
  geom_point(aes(shape = diagnosis))

parkinsons_dataset %>% 
  ggplot(aes(x = id, y = rel_loud_resp2)) +
  geom_boxplot(aes(color = diagnosis)) +
  geom_point(aes(shape = diagnosis))
```
...
parkinsons_dataset %>% 
  ggplot(aes(x = id, y = dur_unvoiced_stops1)) +
  geom_boxplot(aes(color = pd)) +
  geom_point(aes(shape = pd))
...
```{r}
# is there a correlation between diagnosis, age, age onset and updrs: no?
logistic_model_1 <- parkinsons_dataset[1:80,] %>% 
  glm(diagnosis ~ age + age_onset + updrs, family = binomial, data = .)
summary(logistic_model_1)
logistic_model_1 %>% 
  tbl_regression(estimate_fun = function(x) style_number(x, digits = 3), exponentiate = TRUE)

data_split <- parkinsons_dataset %>% 
  drop_na(age_onset) %>% 
  initial_split(prop = 0.7) 
train_data <- training(data_split)
test_data <- testing(data_split)

pred_prob <- logistic_model_1 %>% 
  predict(test_data, type = "response")
predicted.classes <- ifelse(pred_prob > 0.5, "1", "0")
mean(predicted.classes == test_data$diagnosis)
# result: 0 :(

# is there a correlation between updrs, age, age onset and diagnosis: no?
logistic_model_2 <- parkinsons_dataset %>% 
  glm(diagnosis ~ age + duration + updrs, family = binomial, data = .)
...
```{r}
# voice variable tests
# there is NO CORRELATION WHATSOEVER
logistic_model_voice1 <- parkinsons_dataset %>% 
  glm(pd ~ dur_unvoiced_stops1 + dur_unvoiced_stops2, family = binomial, data = .)
summary(logistic_model_voice1)

logistic_model_voice2 <- parkinsons_dataset %>% 
  glm(pd ~ decay_fric1 + decay_fric2, family = binomial, data = .)
summary(logistic_model_voice2)

logistic_model_voice3 <- parkinsons_dataset %>% 
  glm(pd ~ rel_loud_resp1 + rel_loud_resp2, family = binomial, data = .)
summary(logistic_model_voice3)

logistic_model_voice_all <- parkinsons_dataset %>% 
  glm(pd ~ entropy1 + rate_speech_time1 + accel_speech_time1 + dur_pause_int1 + dur_voice_int1 + gap_voiced_int1 + dur_unvoiced_stops1 + decay_fric1 + rel_loud_resp1 + pause_int_resp1 + rate_speech_resp1 + latency_resp1 + entropy2 + rate_speech_time2 + accel_speech_time2 + dur_pause_int2 + dur_voice_int2 + gap_voiced_int2 + dur_unvoiced_stops2 + decay_fric2 + rel_loud_resp2 + pause_int_resp2 + rate_speech_resp2 + latency_resp2, family = binomial, data = .)
summary(logistic_model_voice_all)

logistic_model_rate_speech <- parkinsons_dataset %>% 
  glm(pd ~ rate_speech_resp1 + rbd, family = binomial, data = .)
summary(logistic_model_rate_speech)
```

```{r}
# random graph for eda
pd_test <- parkinsons_dataset %>%
  pivot_longer(cols = 45:68, names_to = "variable", values_to = "value")

facet_labels <- c(
  accel_speech_time1 = "Acceleration of Speech Timing 1",
  accel_speech_time2 = "Acceleration of Speech Timing 2",
  decay_fric1 = "Decay of Unvoiced Fricatives 1",
  decay_fric2 = "Decay of Unvoiced Fricatives 2",
  dur_pause_int1 = "Duration of Paused Intervals 1",
  dur_pause_int2 = "Duration of Paused Intervals 2",
  dur_unvoiced_stops1 = "Duration of Unvoiced Stops 1",
  dur_unvoiced_stops2 = "Duration of Unvoiced Stops 2",
  dur_voice_int1 = "Duration of Voiced Stops 1",
  dur_voice_int2 = "Duration of Voiced Stops 2",
  entropy1 = "Entropy of Speech Timing 1",
  entropy2 = "Entropy of Speech Timing 2",
  gap_voiced_int1 = "Gaping in-between Voiced Intervals 1",
  gap_voiced_int2 = "Gaping in-between Voiced Intervals 2",
  latency_resp1 = "Latency of Respiratory Exchange 1",
  latency_resp2 = "Latency of Respiratory Exchange 2",
  pause_int_resp1 = "Pause Intervals per Respiration 1",
  pause_int_resp2 = "Pause Intervals per Respiration 2",
  rate_speech_resp1 = "Rate of Speech Respiration 1",
  rate_speech_resp2 = "Rate of Speech Respiration 2",
  rate_speech_time1 = "Rate of Speech Timing 1",
  rate_speech_time2 = "Rate of Speech Timing 2",
  rel_loud_resp1 = "Relative Loudness of Resperation 1",
  rel_loud_resp2 = "Relative Loudness of Resperation 2"
)

pd_test %>% 
  ggplot(aes(x = value, y = pd, fill = pd)) +
  geom_boxplot(alpha = 0.8) +
  scale_fill_manual(values = c("deepskyblue","orange"), labels = c("TRUE" = "PD", "FALSE" = "No PD")) +
  scale_y_discrete(labels = c("TRUE" = "PD", "FALSE" = "No PD")) +
  facet_wrap(~ variable, scales = "free_x", nrow = 6, ncol = 4, labeller = labeller(variable = facet_labels)) +
  labs(x = NULL, y = "Status of Parkinson's Disease",  title = "Vocal Data of Patients with PD Compared to Those Without PD", fill = NULL) +
  theme_bw()
```
