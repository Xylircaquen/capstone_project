...
ggplot(just_park, aes(x = duration, y = speech, color=diagnosis)) + 
         geom_jitter()

dur_rigidLLE_model <- glm(duration ~ rigid_LLE, data = just_park)
summary(dur_rigidLLE)

dur_rapidAltMove_RUE_model <- glm(duration ~ rapidAltMove_RUE, data = just_park)
summary(dur_rapidAltMove_RUE_model)

dur_legAg_RLE_model <- glm(duration ~ legAg_RLE, data = just_park)
summary(dur_legAg_RLE_model)

dur_alltremors_model <- glm(duration ~ tremorRest_head + tremorRest_RUE + tremorRest_LUE + tremorRest_LLE + tremorRest_RLE, data = just_park)
summary(dur_alltremors_model)

model <- lm(diagnosis ~ entropy1, data = parkinsons_dataset)
summary(model)
...
logistic_model1 <- glm(pd ~ entropy1 + rate_speech_time1 + accel_speech_time1 + dur_pause_int1 + dur_voice_int1 + gap_voiced_int1 + dur_unvoiced_stops1 + decay_fric1 + rel_loud_resp1 + pause_int_resp1 + rate_speech_resp1 + latency_resp1, family='binomial', data=train_data)
summary(logistic_model1)
logistic_model1 %>% tbl_regression(estimate_fun = function(x) style_number(x, digits = 3), exponentiate = TRUE)
pred_prob1 <- logistic_model1 %>% 
  predict(test_data,type="response")
predicted.classes1 <- ifelse(pred_prob1 > 0.5, "TRUE", "FALSE")
mean(predicted.classes1 == test_data$pd)
...
predictors_speech1 <- data.matrix(parkinsons_dataset[, c('entropy1', 'rate_speech_time1', 'accel_speech_time1', 'dur_pause_int1', 'dur_voice_int1', 'gap_voiced_int1', 'dur_unvoiced_stops1', 'decay_fric1', 'rel_loud_resp1', 'pause_int_resp1', 'rate_speech_resp1', 'latency_resp1')])
library(glmnet)
...
ggplot(parkinsons_dataset, aes(x = dur_pause_int1, y = rate_speech_time1 , color=pd)) + 
         geom_jitter()
